...
We run the analysis on WALS and Grambank in parallel; this works given that the number of processor cores is 16 and
the number of GPUs is 2: within each dataset, 8 processor cores are utilized to parallelize over the 4 chains x 2 runs
of the MrBayes MCMC.

With different hardware, different settings will most likely be needed to ensure optimal performance. In particular,
tune the number of MPI processes started in `src/code/runMrBayes.jl` and the `NPROCS` variable in `Makefile` to suit.

With our hardware and the above assumptions, executing the following script begins the analysis:
