...
fpairs.charNum = 1:nrow(fpairs)
transform!(fpairs, :Column1 => (c -> replace.(c, "-" => " & ")) => :pair)

#meta = deserialize("../../../tmp/$dataset/grid.jls")
meta = CSV.read("../../../tmp/$dataset/grid.csv", DataFrame)
meta = meta[:, [:pair, :class]]
meta = unique(meta)
transform!(meta, :pair => (p -> split.(p, " & ")) => [:f1, :f2])
...
for (k, v) in fDict
  meta.f1 = replace(meta.f1, k => v)
  meta.f2 = replace(meta.f2, k => v)
end

transform!(meta, [:f1, :f2] => ((a,b) -> a .* " & " .* b) => :pair)

allofit = rightjoin(fpairs, meta, on=:pair)


# get rid of controls
whatwewant = filter(:class => (c -> c .!= "control"), allofit)
...
##
nrc = findall(occursin.("NRc", whatwewant.pair))

##
eqs_ = []

for charNum = whatwewant.charNum
...
insertcols!(eqs, 1, :fpair => whatwewant.pair)
...
insertcols!(cors, 1, :fpair => whatwewant.pair)
...
for charNum in whatwewant.charNum
      plot!(
            p,
...
yticks!(p, (1:no_feature_pairs, cors.fpair))
vline!(p,[0])
...
bf = DataFrame(:fpair => whatwewant.pair, :bf => bf_)
...
#=
scatter(bf.bf, 1:no_feature_pairs,
    legend = false,
    yticks= (1:no_feature_pairs,bf.fpair),
