...
klops_W <- klops[klops$dataset == "WALS" & klops$k %in% round(exp(seq(from=log(1), to=log(500), length.out=25))), ]
klops_G <- klops[klops$dataset == "Grambank" & klops$k %in% round(exp(seq(from=log(1), to=log(500), length.out=20))), ]
...
png("../../results/plots/SNR_complex.png", width=3000, height=4500, res=600)
