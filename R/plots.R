...
g <- g + annotate("text", x=90, y=0.008, parse=TRUE, label="italic(p)==0.01", size=3, alpha=0.9)
g <- g + annotate("text", x=90, y=0.04, parse=TRUE, label="italic(p)==0.05", size=3, alpha=0.9)
...
#g <- g + theme(legend.position=c(0.38, 0.80))
g <- g + theme(legend.position="none")
g <- g + annotate("text", x=15, y=0.008, parse=TRUE, label="italic(p)==0.01", size=3, alpha=0.9)
g <- g + annotate("text", x=15, y=0.04, parse=TRUE, label="italic(p)==0.05", size=3, alpha=0.9)
g <- g + scale_color_sanguche2()
g <- g + ylab(expression(italic(p)*"-value"))
g <- g + xlab(expression("Neighbourhood size"~italic(k)))
