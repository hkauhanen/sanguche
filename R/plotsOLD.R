...
wals_meanmean <- mean(data[data$dataset == "WALS" & data$k == 25, ]$mean_distance)
grambank_meanmean <- mean(data[data$dataset == "Grambank" & data$k == 21, ]$mean_distance)
meanmeans <- data.frame(dataset=c("WALS", "Grambank"), x=c(wals_meanmean, grambank_meanmean), k=c(20, 42))

g <- ggplot(data[data$k %in% c(1, 10, 25, 100), ], aes(x=mean_distance, lty=factor(k), color=factor(k), fill=factor(k), group=factor(k))) 
